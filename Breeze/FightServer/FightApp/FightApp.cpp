...
void FightApp::Init(std::string service_id, std::string service_name, std::string file)
{
	this->service_name_ = service_name;
	this->service_id_ = service_id;

	this->node_id_.service_ = SERVICE_FIGHT;
	this->node_id_.id_ = ::atoi(service_id.c_str());

	this->LoadConfig(file);
}

void FightApp::Start(void)
{
		...
		ite++)
	{
		...
		//在FightServer注册 SERVICE_GATEWAY 网关服务
		...
		std::string validate = "FightServer";
		buffer << validate;
...
FREEZE_NET::Ctx& FightApp::GetCtx()
{
	return this->ctx_;
}

bool FightApp::LoadConfig(std::string file)
{
...
void FightApp::AddSession(NetNodeId node, uint32_t serial_num)
{
...
void FightApp::RemoveSession(uint32_t serial_num)
{
	std::map<uint32_t, NetNodeId>::iterator ite_serial = session_node_map_.find(serial_num);
	if (ite_serial != session_node_map_.end())
	{
...
uint32_t FightApp::FindSessionSerial(NetNodeId node)
{
	std::map<NetNodeId, uint32_t>::iterator ite_node = node_session_map_.find(node);
	if (ite_node == node_session_map_.end())
	{
...
NetNodeId FightApp::FindSessionNode(uint32_t serial_num)
{
...
bool FightApp::SendTo(NetNodeId node, unsigned char* ptr, uint32_t ptr_size, ProtocolHead protocol_haad)
{
	std::map<NetNodeId, uint32_t>::iterator ite_node = node_session_map_.find(node);
	if (ite_node == node_session_map_.end())
	{
...
bool FightApp::SendToClient(uint32_t role_id, unsigned char* ptr, uint32_t ptr_size, ProtocolHead protocol_haad)
{
	...
	protocol_haad.destination_service_ = SERVICE_CLIENT;
	protocol_haad.destination_session_serial_num_ = role_id;
...
bool FightApp::SendToRoleServer(unsigned char* ptr, uint32_t ptr_size, ProtocolHead protocol_haad)
{
	...
	protocol_haad.destination_service_ = SERVICE_ROLE;
...
std::map<NetNodeId, NetNodeId>& FightApp::GetClientLinkingGateway(void)
{
	return this->client_linking_gateway_;
}

std::map<uint8_t, std::set<NetNodeId> >& FightApp::GetGatewayLinkingService(void)
{
	return this->gateway_linking_serveice_;
}

void FightApp::InsertGatewayLinkingService(NetNodeId node)
{
...
std::map<uint32_t, uint32_t>& FightApp::GetSessionRoleMap(void)
{
	return this->session_role_map_;
}

std::set<uint32_t>& FightApp::GetOnlineRoleSet(void)
{
